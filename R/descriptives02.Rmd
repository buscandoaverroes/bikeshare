...
data1main <- reactive({ withProgress(message = 'loading1', 
  d() %>% 
...
data1w <- reactive({ withProgress(message = 'loading2', 
  d() %>% 
  select(date, maxtemp) %>%
...
# make the graph, depending on the go, button
  ...
  p1 <- eventReactive(input$go.y1, {
    withProgress(message = "Loading3", # start here.
...
# render the graph
renderPlotly({withProgress(p1(), message = 'loading4')})
